    ...
    return (
      <div>
        ...
        <div className="card overflow-hidden mb-3">
          ...
          {girls.map((r) => (
            <Row key={r.id} row={r} editable={editable} setRows={setRows} updateField={updateField} setVideoRow={setVideoRow} />
          ))}
          {editable && (
...
function Row({
  ...
  row: MovementRow;
  editable: boolean;
  setRows: Dispatch<SetStateAction<MovementRow[]>>;
  updateField: (row: MovementRow, field: "movimento" | "start_val" | "atual" | "meta", value: string) => void;
  setVideoRow: (row: MovementRow) => void;
}) {
